...
def log_sum_exp(vec, m_size):
    ...
    return max_score.view(-1, m_size) + torch.log(torch.sum(torch.exp(vec - max_score.expand_as(vec)), 1)).view(
        -1, m_size)  # B * M  #这里为什么要减max_scorce啊？
...
class CRF(nn.Module):
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        feats = feats.transpose(1, 0).contiguous().view(ins_num, 1,
                                                        tag_size).expand(ins_num, tag_size,
                                                                         tag_size)  #(i,t+2,t+2) 第2维t+2的每一个是一样的
        ...
        )  # bat_size * from_target_size * to_target_size  (b,t+2,t+2) inivalues是每个句子的第一个字
        ...
        for idx, cur_values in seq_iter:
            ...
            cur_values = cur_values + partition.contiguous().view(batch_size, tag_size, 1).expand(
                batch_size, tag_size, tag_size)  #加上上一转移的概率，即，从dim1的某一点出发的话，其转移概率要加上上一转移到达该点的概率（因此对dim2的每一到达点所加概率一样）
        ...
        cur_values = self.transitions.view(1, tag_size, tag_size).expand(
            batch_size, tag_size, tag_size) + partition.contiguous().view(batch_size, tag_size, 1).expand(
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        partition_history = torch.cat(partition_history,
                                      dim=0).view(seq_len, batch_size,
                                                  -1).transpose(1, 0).contiguous()  ## (batch_size, seq_len, tag_size)
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        tg_energy = torch.gather(scores.view(seq_len, batch_size, -1), 2,
                                 new_tags).view(seq_len, batch_size)  # seq_len * bat_size
    ...
    def neg_log_likelihood_loss(self, feats, mask, tags):
        ...
        batch_size = feats.size(0)
        forward_score, scores = self._calculate_PZ(
            feats, mask)  #forward_score:long, scores: (seq_len, batch, tag_size, tag_size)
