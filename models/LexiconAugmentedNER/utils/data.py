...
class Data:
    ...
    def build_gaz_file(self, gaz_file):
        ...
        # 先执行该函数
        if gaz_file:
            # 注意gaz_file是预训练的向量，最后找到fin也是预训练里面的词组，而不是单个汉字；
            fins = open(gaz_file, 'r', encoding="utf-8").readlines()
            for fin in fins:
                fin = fin.strip().split()[0]
                if fin:
                    # print('fin is:{}'.format(fin))
    ...
    def build_gaz_alphabet(self, input_file, count=False):
        ...
        for line in in_lines:
            if len(line) > 3:
                # 正常读入conll格式的数据集，并将word吸入到word_list中；
                ...
                for idx in range(w_length):
                    # 将idx索引及以后的word_list序列输入给enumerateMatchList函数
                    matched_entity = self.gaz.enumerateMatchList(word_list[idx:])
                    # print("匹配结果：{}".format(matched_entity))
                    ...
                    for entity in matched_entity:
                        ...
                        # 如果gaz_count字典中没有找到index对应的值，就将0作为返回结果。
    ...
    def generate_instance_with_gaz(self, input_file, name):
        ...
        if name == "train":
            ...
            self.dev_texts, self.dev_Ids = read_instance_with_gaz(self.HP_num_layer, input_file, self.gaz,
                                                                  ...
                                                                  self.MAX_SENTENCE_LENGTH)
            # print("中文输出样式：")
            # with open("/home/lixu/baseline_ner/LexiconAugmentedNER/data/dev.txt", 'w', encoding='utf-8') as w:
            #     w.write(str(self.dev_texts))
            #     w.close()
            # with open("/home/lixu/baseline_ner/LexiconAugmentedNER/data/words.json", 'w', encoding='utf-8') as w:
            #     w.write(str(self.word_alphabet.instance2index))
            #     w.close()
            # with open("/home/lixu/baseline_ner/LexiconAugmentedNER/data/chars.json", 'w', encoding='utf-8') as w:
            #     w.write(str(self.char_alphabet.instance2index))
            #     w.close()
            # with open("/home/lixu/baseline_ner/LexiconAugmentedNER/data/biwords.json", 'w', encoding='utf-8') as w:
            #     w.write(str(self.biword_alphabet.instance2index))
            #     w.close()
        elif name == "test":
