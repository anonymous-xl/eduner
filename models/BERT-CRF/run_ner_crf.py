...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        json_d = {}
        # 将输入的文本tensor还原成文本输出，便于查看哪些预测错误了。
        print(batch[0][0].cpu().numpy())
        sent_content = tokenizer.batch_decode(batch[0])
        json_d['sent'] = sent_content
        json_d['id'] = step
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        'cached_crf-{}_{}_{}_{}'.format(data_type,
...
def main():
    ...
    label_list = processor.get_labels()
    # print("看一下读取的labels有:{}", label_list)
    ...
    model = model_class.from_pretrained(args.model_name_or_path, config=config, from_tf=False)
