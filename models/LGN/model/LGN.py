...
class Graph(nn.Module):
        ...
        if self.bidirectional:
            ...
            if self.use_global:
                ...
                self.node_rnn_b = Nodes_Cell(self.hidden_dim * 3,
                                             ...
                                             dropout=self.cell_dropout_rate)
                if self.use_edge:
                    self.edge_rnn_b = Edges_Cell(self.hidden_dim * 2,
                                                 self.hidden_dim,
                                                 use_global=False,
                                                 dropout=self.cell_dropout_rate)
    ...
    def construct_graph(self, batch_size, seq_len, word_list):
        ...
        for sen in range(batch_size):
            sen_nodes_mask = torch.zeros([1, seq_len], device=device).bool()
            sen_words_length = torch.zeros([1, self.length_dim], device=device)
            sen_words_mask_f = torch.zeros([1, seq_len], device=device).bool()
            sen_words_mask_b = torch.zeros([1, seq_len], device=device).bool()
            ...
            for w in range(seq_len):
                if w < len(word_list[sen]) and word_list[sen][w]:
                    for word, word_len in zip(word_list[sen][w][0], word_list[sen][w][1]):
                        ...
                        nodes_mask = torch.ones([1, seq_len], device=device).bool()
                        words_mask_f = torch.ones([1, seq_len], device=device).bool()
                        words_mask_b = torch.ones([1, seq_len], device=device).bool()
        ...
        batch_words_mask_f = torch.ones([batch_size, max_edge_num, seq_len], device=device).bool()
        batch_words_mask_b = torch.ones([batch_size, max_edge_num, seq_len], device=device).bool()
        batch_words_length = torch.zeros([batch_size, max_edge_num, self.length_dim], device=device)
        if self.use_edge:
            batch_nodes_mask = torch.zeros([batch_size, max_edge_num, seq_len], device=device).bool()
    ...
    def update_graph(self, word_list, word_inputs, mask):
        ...
        for i in range(self.iters):
            ...
            nodes_begin_f = torch.sum(nodes_f[:, None, :, :] * (~words_mask_b)[:, :, :, None].float(), 2)
            nodes_begin_f = torch.cat([torch.zeros([B, 1, H], device=nodes_f.device), nodes_begin_f[:, 1:N, :]], 1)

            if self.use_edge:
                nodes_att_f = self.edge2node_f[i](nodes_f, torch.cat([edges_f, nodes_begin_f, words_length],
                                                                     -1).unsqueeze(2), words_mask_f)
                if self.use_global:
                    glo_att_f = torch.cat([
                        self.glo_att_f_node[i](glo_f, nodes_f,
                                               (1 - mask).bool()), self.glo_att_f_edge[i](glo_f, edges_f,
                                                                                          (1 - edges_mask).bool())
                    ], -1)
            else:
                nodes_att_f = self.edge2node_f[i](nodes_f, torch.cat([nodes_begin_f, words_length], -1).unsqueeze(2),
                                                  words_mask_f)
                if self.use_global:
                    glo_att_f = self.glo_att_f_node[i](glo_f, nodes_f, (1 - mask).bool())
            ...
            if self.use_edge and N > 1:
                if self.use_global:
                    edges_f = torch.cat([
                        edges_f[:, 0:1, :],
                        self.edge_rnn_f(edges_f[:, 1:N, :], edges_att_f[:, 1:N, :], glo_att_f.expand(B, N - 1, H * 2))
                    ], 1)
                else:
                    edges_f = torch.cat(
                        [edges_f[:, 0:1, :],
                         self.edge_rnn_f(edges_f[:, 1:N, :], edges_att_f[:, 1:N, :])], 1)

                edges_f_cat = torch.cat([edges_f_cat, edges_f[:, None, :, :]], 1)
                edges_f = torch.cat([edges_f[:, 0:1, :], self.norm(torch.sum(edges_f_cat[:, :, 1:N, :], 1))], 1)

            nodes_f_r = torch.cat(
                [torch.zeros([B, 1, self.hidden_dim], device=nodes_f.device), nodes_f[:, 0:(L - 1), :]], 1)
        ...
        if self.bidirectional:
            ...
            for i in range(self.iters):
                ...
                nodes_begin_b = torch.sum(nodes_b[:, None, :, :] * (~words_mask_f)[:, :, :, None].float(), 2)
                nodes_begin_b = torch.cat([torch.zeros([B, 1, H], device=nodes_b.device), nodes_begin_b[:, 1:N, :]], 1)

                if self.use_edge:
                    nodes_att_b = self.edge2node_b[i](nodes_b, torch.cat([edges_b, nodes_begin_b, words_length],
                                                                         -1).unsqueeze(2), words_mask_b)
                    if self.use_global:
                        glo_att_b = torch.cat([
                            self.glo_att_b_node[i](glo_b, nodes_b,
                                                   (1 - mask).bool()), self.glo_att_b_edge[i](glo_b, edges_b,
                                                                                              (1 - edges_mask).bool())
                        ], -1)
                else:
                    nodes_att_b = self.edge2node_b[i](nodes_b, torch.cat([nodes_begin_b, words_length],
                                                                         -1).unsqueeze(2), words_mask_b)
                    if self.use_global:
                        glo_att_b = self.glo_att_b_node[i](glo_b, nodes_b, (1 - mask).bool())
                ...
                if self.use_edge and N > 1:
                    if self.use_global:
                        edges_b = torch.cat([
                            edges_b[:, 0:1, :],
                            self.edge_rnn_b(edges_b[:, 1:N, :], edges_att_b[:, 1:N, :], glo_att_b.expand(
                                B, N - 1, H * 2))
                        ], 1)
                    else:
                        edges_b = torch.cat(
                            [edges_b[:, 0:1, :],
                             self.edge_rnn_b(edges_b[:, 1:N, :], edges_att_b[:, 1:N, :])], 1)

                    edges_b_cat = torch.cat([edges_b_cat, edges_b[:, None, :, :]], 1)
                    edges_b = torch.cat([edges_b[:, 0:1, :], self.norm(torch.sum(edges_b_cat[:, :, 1:N, :], 1))], 1)

                nodes_b_r = torch.cat([nodes_b[:, 1:L, :],
                                       torch.zeros([B, 1, self.hidden_dim], device=nodes_b.device)], 1)
