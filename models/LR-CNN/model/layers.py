...
class GlobalGate(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.self_attention2 = MultiHeadAttention(self.head, self.hidden_dim, self.hidden_dim // self.head,
                                                  self.hidden_dim // self.head)
...
class LayerGate(nn.Module):
    def __init__(self, hidden_dim, input_dim, use_gaz=True, gpu=True):
        ...
        self.index = torch.LongTensor([[i + j * hidden_dim * 4 for i in range(self.hidden_dim * 4)]
                                       for j in range(4)])  #(4,H) [[0:H],[h:2H],[2H:3H],[3H:4H]]  H=4h
        self.index2 = torch.LongTensor([[i + j * hidden_dim * 3 for i in range(self.hidden_dim * 3)]
                                        for j in range(4)])  #(4,H) H=3h
        if gpu:
...
class MultiscaleAttention(nn.Module):
    def __init__(self, num_layer, dropout):
        super().__init__()
        self.MLP_layer = nn.Sequential(nn.Dropout(p=dropout), nn.Linear(num_layer, num_layer), nn.Tanh(), nn.Dropout(p=dropout),
                                       nn.Linear(num_layer, num_layer), nn.Tanh())
