...
def load_data():
    ...
    if dataset == 'ontonotes':
        paths = {
            'train': '../data/OntoNote4NER/***',
            "dev": '../data/OntoNote4NER/dev.char.bmes',
            "test": '../data/OntoNote4NER/test.char.bmes'
        }
        min_freq = 2
    elif dataset == 'weibo':
        paths = {'train': '../data/Weibo/***', 'dev': 'data/Weibo/dev.csv', 'test': 'data/Weibo/test.csv'}
        min_freq = 1
    elif dataset == 'resume':
        paths = {
            'train': 'data/ResumeNER/train.csv',
            'dev': 'data/ResumeNER/dev.csv',
            'test': 'data/ResumeNER/test.csv'
        }
        min_freq = 1
    elif dataset == 'msra':
        paths = {
            'train': '../data/MSRANER/train_dev.***',
    ...
    embed = StaticEmbedding(data_bundle.get_vocab('chars'),
                            model_dir_or_name='embeddings/gigaword_chn.all.a2b.uni.ite50.vec',
                            min_freq=1,
    ...
    bi_embed = StaticEmbedding(data_bundle.get_vocab('bigrams'),
                               model_dir_or_name='embeddings/gigaword_chn.all.a2b.bi.ite50.vec',
                               word_dropout=0.02,
                               dropout=0.3,
                               min_freq=min_freq,
                               only_norm_found_vector=normalize_embed,
